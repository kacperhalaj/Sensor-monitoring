...
class Logger:
    ...
    def start(self) -> None:
        ...
        if not self.current_file or self.current_file.name != file_path:
    ...
    def _check_rotation(self) -> None:
        ...
        if self.current_file:
            file_size = os.path.getsize(self.current_file.name) / (1024 * 1024)
            file_line_count = sum(1 for _ in open(self.current_file.name))

            if file_size >= self.max_size_mb or file_line_count >= self.rotate_after_lines:
                self._rotate()

    def _rotate(self) -> None:
        """Wykonuje rotację pliku logu."""
        self.stop()  # Zamyka bieżący plik
        self._archive()  # Archiwizuje stary plik
        self.start()  # Tworzy nowy plik
